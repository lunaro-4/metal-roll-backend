...
from .handlers import (get_all_coil,
                       printerr,
                       handle_add_coil,
                       handle_delete_coil,
                       handle_get_coil_stats,
                       separate_stats_data,
                       count_by_param,
                       calculate_stats_from_list,
                       calculate_sum_coil_weight,
                       find_gaps
                       )
...
@router.get("/coil")
async def get_coil(id_start: int | None = None,
                   ...
                   add_date_start: str | None = None,
                   add_date_end: str | None = None,
                   del_date_start: str | None = None,
                   del_date_end: str | None = None,
    ...
    coil_response = await get_all_coil(session=session,
                                       ...
                                       add_date_start=add_date_start,
                                       add_date_end=add_date_end,
                                       del_date_start=del_date_start,
                                       del_date_end=del_date_end)
    return coil_response


@router.post("/coil")
async def create_coil(coil: CoilModel,
                      session: AsyncSession = Depends(get_db)):
    add_date = None
    del_date = None
    if coil.weight < 0 or coil.length < 0:
        raise HTTPException(status_code=400,
                            detail="Weight and length must be non-negative")
    try:
        if coil.add_date is not None and coil.add_date != '':
            add_date = datetime.strptime(coil.add_date,
                                         DATE_FORMAT)
        if coil.del_date is not None and coil.del_date != '':
            del_date = datetime.strptime(coil.del_date,
                                         DATE_FORMAT)
        if (del_date is not None
                and add_date is not None
                and add_date > del_date):
        ...
        printerr(ae)
        raise HTTPException(status_code=400,
                            detail="Item was deleted before added")
    except Exception as e:
        printerr(e)
        raise HTTPException(status_code=503,
                            detail="Unexpected error occured")

    response = await handle_add_coil(coil=coil,
                                     session=session, add_date=add_date)
    return response
...
async def get_coil_stats(session: AsyncSession = Depends(get_db),
    ...
    coil_stats = await handle_get_coil_stats(datetime_start,
                                             datetime_end,
                                             session)

    (coil_length_list,
     coil_weight_list,
     coil_add_del_dict) = separate_stats_data(coil_stats)

    add_amount = await count_by_param(start_date=datetime_start,
                                      end_date=datetime_end,
                                      session=session, param=CoilBase.add_date)
    del_amount = await count_by_param(start_date=datetime_start,
                                      end_date=datetime_end,
                                      session=session,
                                      param=CoilBase.del_date)

    (coil_len_max,
     coil_len_min,
     coil_len_avg,
     coil_len_sum) = calculate_stats_from_list(coil_length_list)
    (coil_weight_max,
     coil_weight_min,
     coil_weight_avg,
     coil_weight_sum) = calculate_stats_from_list(coil_weight_list)

    (coil_weight_sum_fixed,
     max_weight_date,
     min_weight_date) = (await
                         calculate_sum_coil_weight(start_date=datetime_start,
                                                   end_date=datetime_end,
                                                   session=session))
